// Landmark Mania — the second "Traffic Time" game. Shares Truck Mania's board
...
const LOC_LABELS = {
  ...
  upgrade: "Upgrade spot", discovery: "Discovery", star: "Star",
  landmark: "Landmark", required: "Required"
};
// The payout symbol shown inside a location's empty token circles. A forced-
// mode required location pays nothing, so its circle carries no symbol at all
// — the color IS the message.
const SLOT_SYMBOLS = {
  timestone: "⬟", token: "💰", destress: "🍵", discovery: "🔭", star: "⭐",
  required: ""
};

// The seats' colors, in seat order. (Server: PLAYER_COLORS — keep in sync.)
// Forced mode's board is dealt for all five whether or not five are playing,
// so circles belonging to an empty seat still have to be paintable — hence a
// list here rather than only reading playersState.
const PLAYER_COLORS = ["#3ac0c0", "#e0559c", "#e0a13a", "#7b6fe0", "#57b947"];
const seatColor = (i) => playersState[i]?.color ?? PLAYER_COLORS[i] ?? "#8d867b";
// Is this seat actually at the table? A forced-mode circle painted for a seat
// nobody is playing is decoration — nobody can ever use it.
const seatPlaying = (i) => i != null && i < playersState.length;

// ---- Forced mode: how a location shows whose it is -------------------------
// Three independent channels, any combination (including none). They're a
// VIEW preference, not a game rule — kept per-browser rather than in the
// room's settings, so flipping one redraws instantly and never re-deals the
// board or changes what anyone else is looking at.
//   circle  — the token circle carries the color(s): a solid ring for a
//             required location, split diagonally for a shared one.
//   outline — the building's border carries them; a shared location splits
//             the perimeter in half, which on a rectangle is two sides each.
//   fill    — the building's fill carries them, again split on the diagonal.
// Outline and fill each displace the neighbourhood color in that channel, so
// with both on the neighbourhoods have nowhere left to show at all — unless
// BLOCKS is on, which is the fourth channel and the odd one out: it says
// nothing about players. It paints the ground between the streets by
// neighbourhood, which frees the buildings from having to carry that too, so
// every player channel that's switched OFF falls back to plain stone instead
// of a hood color.
// CORNER is the odd one out again, and it wins: a whole layout rather than a
// channel. The building is given over to its name and a big picture, the
// player colors take the border (strong) and the fill (light), and the token
// circle shrinks back down into the bottom-right corner with no color on it
// at all — black outline, white inside — so it reads as a slot to be filled
// rather than as anybody's property. Neighbourhoods don't appear anywhere.
// With it on the four channels above are ignored, and the menu greys them out
// to say so.
const FORCED_PAINT = [
  ["circle", "⭕ Circle", "The token circle carries the player color — a ring for a required location, split diagonally for a shared one."],
  ["outline", "▢ Outline", "The building's border carries the player color. A shared location splits the perimeter in half — two of the four sides each on a rectangle."],
  ["fill", "◧ Fill", "The building's fill carries the player color, split diagonally for a shared location."],
  ["blocks", "▦ Blocks", "The ground between the streets is painted by neighbourhood. With this on, buildings drop the hood colors entirely — any channel that isn't showing a player goes plain stone."],
  ["corner", "◱ Corner", "A layout of its own, and it overrides the four above: name up top, a big picture in the middle, strong player color on the border and a light wash of it inside, and a small colorless circle in the bottom-right corner. Neighbourhoods aren't shown at all."]
];
// Corner overrides everything else, so every other channel asks through here.
const isCornerPaint = () => isForcedMode() && forcedPaint.corner === true;
// What a building wears in a channel that's showing nobody, once the blocks
// channel has taken the neighbourhoods over.
const UM_NEUTRAL = "#cfc9bd";
let forcedPaint = (() => {
  const fallback = { circle: true, outline: false, fill: false, blocks: false, corner: false };
  try {
    const raw = JSON.parse(localStorage.getItem("umForcedPaint") ?? "null");
    if (!raw || typeof raw !== "object") return fallback;
    return {
      circle: !!raw.circle, outline: !!raw.outline, fill: !!raw.fill,
      blocks: !!raw.blocks, corner: !!raw.corner
    };
  } catch {
    return fallback;
  }
})();
const paintOn = (channel) => isForcedMode() &&
  (channel === "corner" ? forcedPaint.corner === true
    : !isCornerPaint() && forcedPaint[channel] === true);
// The seats a forced-mode location belongs to: one for a required circle, two
// for a shared one, none for anything else.
const locSeats = (b) => (Array.isArray(b?.pair)
  ? b.pair.slice(0, 2)
  : b?.owner != null ? [b.owner] : []);
...
// Star mode: 52 locations, single circles wiped every dawn, five
// neighbourhoods scored daily, and a star rating that sets what each landmark
// card pays. (Keep the numbers in sync with the server.)
const isStarMode = () => settingsState?.starMode === true;
// Simple mode: 50 locations in five even neighbourhoods (1 time stone, 1
// token, 2 discovery and 6 landmarks apiece), single circles wiped every dawn,
// no upgrades. 6 day / 6 night of the 20 token locations keep the clock
// turning. Everything scores through a five-star meter: each day's end
// pays up to two stars (the spread, and a clean hand) and the final score is
// landmarks completed × stars. (Keep the numbers in sync with the server.)
const isSimpleMode = () => settingsState?.simpleMode === true;
// Forced mode: 50 locations in seven decorative neighbourhoods, every one with
// a single circle. Thirty are REQUIRED — the circle wears one player's color,
// only they may use it, it pays nothing, and leaving it empty at the end costs
// them a point. The other twenty are SHARED — the circle is split diagonally
// between two players' colors, only those two may use it, and whichever of
// them gets there first takes the full reward while the other is handed half
// on the spot. Cards run on duplicate plumbing (every location is a possible
// destination, and arriving is a choice of card OR circle, never both), the
// hand is always two, and tokens never leave the board.
// (Keep the numbers here in sync with the server.)
const isForcedMode = () => settingsState?.forcedMode === true;
const isNewMode = () =>
  isLandmarkMode() || isStarMode() || isSimpleMode() || isForcedMode();
const RATING_MAX = 7;
// Points at day's end for the number of different neighbourhoods you hold a
// token in (index = that count) — two neighbourhoods pay nothing, so only a
// real spread is worth chasing.
...
// Landmark mode shows one (two with extra ride) and bills a point per buried
// card each night; star mode shows two and docks half a star instead.
// Forced mode holds nothing face down at all — the hand is always full and
// always showing.
const visibleCardCap = () => (isForcedMode()
  ? Math.max(1, settingsState?.forcedCards ?? 2)
  : isStarMode() || isSimpleMode()
  ? 2
  : 1 + (hasUpgrade("extraRide") ? 1 : 0));
...
// { truckId, spot, facing } — a car to put back with no animation at all (an
// undone move). One-shot: the server only sends it on the undo's own update.
let snapCarState = null;
// Where each car stood just before its current drive: { spot, x, y, angle }.
// An undo puts it back exactly here. The server can't tell us — it only ever
// updates `truck.facing` for AI cars, so a human car's stored facing is stale
// and a parked car is otherwise drawn at its spot's angle, which is not
// necessarily the angle it actually arrived on.
const carUndoPose = {};
...
let lastFunSeq = -1;
// Forced mode: the last split-circle payout ({ seq, from, to, kind, amount }).
let shareState = null;
let lastShareSeq = -1;
...
function isActive() {
  return app.currentGame?.id === "landmark-mania";
}
...
// The blocks channel: the ground between the streets, painted by
// neighbourhood. The map carries each block pre-cut into a handful of
// rectangles that tile it exactly and stop at the kerb, so this can't ever
// paint over a road. A block takes the hood most of its locations belong to
// (they're grouped geographically, so that's nearly always all of them); one
// with no locations at all stays bare.
function renderHoodBlocks(svgArg = null) {
  // During a full map render the svg is still detached, so the caller hands
  // it over rather than us going looking for it.
  const svg = svgArg ?? els.gameBoard.querySelector(".tm-map");
  svg?.querySelector(".um-hood-blocks")?.remove();
  if (!svg || !mapState || !paintOn("blocks")) return;
  const blocksLayer = svg.querySelector(".tm-blocks");
  const layer = svgEl("g", { class: "um-hood-blocks" });
  // Behind the buildings, in front of the streets.
  ...
  for (const block of mapState.blocks ?? []) {
    ...
    for (const b of block.buildings ?? []) {
      if (b.role !== "loc" || b.hood == null) continue;
      tally.set(b.hood, (tally.get(b.hood) ?? 0) + 1);
    }
    if (!tally.size) continue;
    let hood = null;
    let best = 0;
    for (const [id, n] of tally) {
      if (n > best) { best = n; hood = id; }
    }
    const color = hoodsState.find((h) => h.id === hood)?.color;
    if (!color) continue;
    for (const [x, y, w, h] of block.rects) {
      svgEl("rect", { x, y, width: w, height: h, fill: color, class: "um-hood-block" }, layer);
    }
  }
}

// Forced mode's diagonal two-color fill. One gradient per pair of seats — ten
// at most — parked in the map's own defs, with a hard stop at the halfway
// mark so it reads as a split rather than a blend. The axis runs top-left to
// bottom-right, which puts the seam on the other diagonal, matching the way
// the shared circles are cut.
function pairFillGradient(parent, a, b, tint) {
  const svg = parent.ownerSVGElement ?? parent.closest?.("svg");
  if (!svg) return null;
  const id = `um-fillpair-${a}-${b}-${Math.round(tint * 100)}`;
  if (!svg.querySelector(`#${id}`)) {
    const defs = svg.querySelector("defs.um-defs") ?? svgEl("defs", { class: "um-defs" }, svg);
    const grad = svgEl("linearGradient", { id, x1: "0", y1: "0", x2: "1", y2: "1" }, defs);
    svgEl("stop", { offset: "50%", "stop-color": lighten(seatColor(a), tint) }, grad);
    svgEl("stop", { offset: "50%", "stop-color": lighten(seatColor(b), tint) }, grad);
  }
  return `url(#${id})`;
}

// Forced mode's colored border. One seat gets the whole outline; two split it
// by perimeter, half each — which on a rectangle, walked from a corner, comes
// out as exactly two of the four sides apiece.
function paintSplitOutline(g, building, seats, strong = false) {
  const pts = buildingCorners(building);
  if (pts.length < 3 || !seats.length) return;
  const layer = svgEl("g", {
    class: strong ? "um-forced-outline um-forced-outline-strong" : "um-forced-outline"
  }, g);
  if (building.rotation && !building.points) {
  ...
  const total = edges.reduce((n, e) => n + e.len, 0);
  if (!total) return;
  // One color: the whole loop in one stroke.
  if (seats.length < 2) {
    const path = svgEl("path", {
      d: `${pts.map(([x, y], i) => `${i ? "L" : "M"} ${r1(x)} ${r1(y)}`).join(" ")} Z`,
      class: "um-forced-outline-line"
    }, layer);
    path.style.stroke = seatColor(seats[0]);
    return;
  }
  // Two: walk the loop, cutting at the halfway mark (splitting an edge if the
  // mark lands mid-way along one, which is what polygon buildings need).
  const half = total / 2;
  const runs = [[], []];
  let walked = 0;
  for (const e of edges) {
    let done = 0;
    while (done < e.len - 1e-6) {
      const side = walked < half - 1e-6 ? 0 : 1;
      const take = Math.min(e.len - done, side === 0 ? half - walked : Infinity);
      const at = (t) => [e.a[0] + (e.b[0] - e.a[0]) * t, e.a[1] + (e.b[1] - e.a[1]) * t];
      runs[side].push([at(done / e.len), at((done + take) / e.len)]);
      walked += take;
      done += take;
    }
  }
  runs.forEach((segs, k) => {
    ...
    for (const [p0, p1] of segs) {
      const same = cursor && Math.hypot(cursor[0] - p0[0], cursor[1] - p0[1]) < 0.01;
      if (!same) d += `M ${r1(p0[0])} ${r1(p0[1])} `;
      d += `L ${r1(p1[0])} ${r1(p1[1])} `;
      cursor = p1;
    }
    const path = svgEl("path", { d: d.trim(), class: "um-forced-outline-line" }, layer);
    path.style.stroke = seatColor(seats[k]);
  });
}
...
function appendBuilding(parent, building) {
  ...
  // Forced mode: the fill and outline channels each hand their half of the
  // building over to the seat(s) it belongs to, displacing the neighbourhood
  // color there. With both on (and the circle too) the neighbourhood has
  // nowhere left to show, which is the point.
  const seats = isLoc ? locSeats(building) : [];
  // The corner layout takes both color channels itself: a strong border and a
  // light wash inside, with the neighbourhood nowhere to be seen.
  const corner = isCornerPaint() && isLoc && seats.length > 0;
  const paintFill = corner || (paintOn("fill") && seats.length > 0);
  const paintLine = corner || (paintOn("outline") && seats.length > 0);
  // Corner's wash is deliberately paler than the fill channel's — a big
  // picture and a title have to sit on top of it and stay readable.
  const tint = corner ? 0.66 : 0.5;
  // Once the ground is carrying the neighbourhoods (or the corner layout has
  // dropped them entirely), the buildings stop repeating the hood color: a
  // channel showing nobody goes plain stone.
  const base = corner || paintOn("blocks") ? UM_NEUTRAL : building.color;
  // What the fill actually is, and — separately — a flat color to test the
  // name's contrast against, since a two-seat fill is a gradient reference.
  const fillProbe = paintFill
    ? lighten(seatColor(seats[0]), tint)
    : isLoc ? lighten(base) : base;
  const fillColor = (paintFill && seats.length > 1
    ? pairFillGradient(parent, seats[0], seats[1], tint)
    : null) ?? fillProbe;
  // The border, the driveways and their dots all follow the outline channel.
  const edgeColor = paintLine ? seatColor(seats[0]) : base;
  // A split border is drawn by hand below, so the shape's own stroke stands
  // down rather than showing through underneath it.
  if (isLoc) g.style.setProperty("--um-stroke", paintLine ? "transparent" : base);
  ...
  if (paintLine) paintSplitOutline(g, building, seats, corner);
  ...
  if (building.role === "loc" && building.locType === "landmark") {
    // A landmark: its plain name across the top, the matching emoji big
    // underneath. The name is what the card says, so the two read together
    // without anyone having to decode the picture.
    const [cx, cy] = buildingCentroid(building);
    const bb = buildingBBox(building);
    const name = svgEl("text", { x: cx, y: bb.minY + 9, class: "um-loc-name um-landmark-name" }, g);
    name.textContent = building.name ?? "";
    if (isDarkColor(fillProbe)) name.style.fill = "rgba(247, 244, 238, 0.95)";
    const icon = svgEl("text", { x: cx, y: cy + 6, class: "um-loc-emoji" }, g);
  ...
  } else if (building.role === "loc" && isCornerPaint()) {
    // The corner layout: name across the top, the picture big in the middle,
    // and a small colorless circle tucked into the bottom-right. The border
    // and the wash already say whose location this is, so the circle carries
    // no color of its own — it's a slot, black on white, until somebody's
    // token lands in it.
    const [cx] = buildingCentroid(building);
    const bb = buildingBBox(building);
    const geom = slotGeometry(building);
    const [sx, sy] = geom.centers[0];

    const name = svgEl("text", {
      x: cx, y: bb.minY + 9, class: "um-loc-name um-forced-name"
    }, g);
    name.textContent = building.name ?? "";
    if (isDarkColor(fillProbe)) name.style.fill = "rgba(247, 244, 238, 0.95)";

    // The picture, filling what's left. Nudged up and left of dead centre so
    // the corner circle isn't sitting on top of it.
    const icon = svgEl("text", {
      x: cx - 2, y: (bb.minY + 12 + bb.maxY) / 2 - 2, class: "um-forced-emoji"
    }, g);
    icon.style.fontSize = `${Math.max(14, Math.min(30,
      (bb.maxX - bb.minX) * 0.44, (bb.maxY - bb.minY - 12) * 0.72))}px`;
    icon.textContent = building.emoji ?? "📍";

    const slots = svgEl("g", { class: "um-loc-slots" }, g);
    svgEl("circle", {
      cx: sx, cy: sy, r: geom.r, class: "um-slot um-slot-forced um-slot-forced-disc"
    }, slots);
    const sym = svgEl("text", { x: sx, y: sy + 0.5, class: "um-slot-sym" }, slots);
    sym.style.fontSize = `${Math.max(8, Math.round(geom.r * 1.05))}px`;
    sym.textContent = SLOT_SYMBOLS[building.locType] ?? "";

    if (building.period) {
      // Bottom-left, mirroring the circle — the title owns the top edge.
      const badge = svgEl("text", {
        x: bb.minX + 9, y: bb.maxY - 9, class: "um-loc-period um-forced-period"
      }, g);
      badge.textContent = PERIOD_SYMBOLS[building.period] ?? "";
    }
    svgEl("g", { class: "um-loc-under" }, g);
    svgEl("title", {}, g).textContent = forcedLocTitle(building);
  } else if (building.role === "loc" && isForcedMode()) {
    // Forced mode's location: its name across the top, and below it one big
    // circle. That's all — the circle is the whole rule set (whose it is,
    // whether it pays, whether anyone's been), so nothing else competes with
    // it for the lot.
    const [cx] = buildingCentroid(building);
    const bb = buildingBBox(building);
    const geom = slotGeometry(building);
    ...
    // The sun / moon shares the top line, so the title shifts off-centre to
    // make room for it.
    const name = svgEl("text", {
      x: cx - (building.period ? 6 : 0), y: bb.minY + 9, class: "um-loc-name um-forced-name"
    }, g);
    name.textContent = building.name ?? "";
    if (isDarkColor(fillProbe)) name.style.fill = "rgba(247, 244, 238, 0.95)";

    const slots = svgEl("g", { class: "um-loc-slots" }, g);
    // The circle channel. With it off the circle is a plain one — the payout
    // symbol and nothing else — and whose location this is has to be read off
    // the outline or the fill, or not at all.
    const colored = paintOn("circle");
    const pair = colored && Array.isArray(building.pair) ? building.pair : null;
    if (pair) {
      // A shared circle: split along the diagonal, one player's color each
      // side. Both halves stay full strength even once it's used — which
      // player was standing to gain from it is worth being able to read all
      // game, not just until somebody gets there.
      const d = geom.r * Math.SQRT1_2;
      const arc = (sweep) => `M ${r1(sx - d)} ${r1(sy - d)} A ${r1(geom.r)} ${r1(geom.r)} 0 0 ${sweep} ${r1(sx + d)} ${r1(sy + d)} Z`;
      [1, 0].forEach((sweep, k) => {
        const half = svgEl("path", { d: arc(sweep), class: "um-forced-half" }, slots);
        half.style.fill = seatColor(pair[k]);
      });
    } else if (colored && building.owner != null) {
      // A required circle: one player's color, ghosted until they cover it.
      const own = svgEl("circle", { cx: sx, cy: sy, r: geom.r, class: "um-forced-own" }, slots);
      own.style.fill = seatColor(building.owner);
    }
    const ring = svgEl("circle", { cx: sx, cy: sy, r: geom.r, class: "um-forced-ring" }, slots);
    if (colored && !pair && building.owner != null) {
      ring.style.stroke = seatColor(building.owner);
      ring.classList.add("um-forced-ring-own");
    }
    // The token itself — the element refreshLocations paints when the circle
    // is claimed. On a colored shared circle it's the inner disc that carries
    // the payout symbol until somebody's token replaces it; otherwise it fills
    // the whole circle. (A token on the board is who has BEEN somewhere, not
    // whose location it is, so it stays player-colored whatever the channels
    // are set to.)
    const painted = colored && (pair || building.owner != null);
    svgEl("circle", {
      cx: sx, cy: sy, r: r1(pair ? geom.r * 0.58 : geom.r),
      // A disc that has ownership paint UNDER it stays clear so the paint
      // shows; anything else is the plain white circle every other mode uses.
      class: `um-slot um-slot-forced${pair || !painted ? " um-slot-forced-disc" : ""}`
    }, slots);
    const sym = svgEl("text", { x: sx, y: sy + 0.5, class: "um-slot-sym" }, slots);
    // Sized to the disc it actually sits in — the small inner one on a colored
    // shared circle, the whole thing otherwise.
    const discR = pair ? geom.r * 0.58 : geom.r;
    sym.style.fontSize = `${Math.max(9, Math.min(16, Math.round(discR * 1.05)))}px`;
    sym.textContent = SLOT_SYMBOLS[building.locType] ?? "";

    if (building.period) {
      const badge = svgEl("text", {
        x: bb.maxX - 8, y: bb.minY + 9, class: "um-loc-period um-forced-period"
      }, g);
      badge.textContent = PERIOD_SYMBOLS[building.period] ?? "";
    }
    svgEl("g", { class: "um-loc-under" }, g);
...
// What a forced-mode location's circle actually means, in words — the board
// says it in color, and this says it on hover.
function forcedLocTitle(b) {
  const taken = (b.slots ?? [])[0];
  const when = b.period === "day" ? " Open by day only (☀️)."
    : b.period === "night" ? " Open at night only (🌙)." : "";
  if (Array.isArray(b.pair)) {
    const [a, c] = b.pair;
    const kind = b.locType === "timestone" ? "time stones" : "tokens";
    const full = b.locType === "timestone"
      ? (settingsState?.timeStoneReward ?? 6)
      : (settingsState?.tokenReward ?? 4);
    const who = `${seatName(a)} & ${seatName(c)}`;
    if (taken != null) {
      const other = b.pair.find((s) => s !== taken);
      return `${b.name} — ${seatName(taken)} got here first and took ${full} ${kind}; ${seatName(other)} was handed ${Math.floor(full / 2)}. Nobody else could ever use it.`;
    }
    return `${b.name} — shared by ${who}. Whichever of them gets here takes ${full} ${kind}; the other is handed ${Math.floor(full / 2)} on the spot. Only one of them ever gets to use it.${when}`;
  }
  if (b.owner == null) return b.name ?? "";
  const miss = settingsState?.forcedMissPenalty ?? 1;
  if (taken != null) return `${b.name} — covered by ${seatName(b.owner)}. No longer costs them anything.`;
  if (!seatPlaying(b.owner)) {
    return `${b.name} — belongs to a seat nobody is playing, so it just sits there.`;
  }
  return `${b.name} — required for ${seatName(b.owner)}, and nobody else may ever use it. Pays nothing; still empty at the end of the game costs them ${miss} point${miss === 1 ? "" : "s"}.${when}`;
}
...
function slotGeometry(b) {
  ...
  if (isCornerPaint()) {
    // The corner layout: the circle is only a slot to be filled, so it shrinks
    // back out of the way into the bottom-right corner and leaves the lot to
    // the name and the picture. Whose location this is comes off the border
    // and the wash instead.
    const bb = buildingBBox(b);
    const r = Math.max(6, Math.min(10, (bb.maxX - bb.minX) * 0.2, (bb.maxY - bb.minY) * 0.2));
    const x = bb.maxX - r - 3;
    const y = bb.maxY - r - 3;
    return { r, sym: Math.max(8, Math.round(r * 1.05)), underY: y + r + 4, centers: [[x, y]] };
  }
  if (isForcedMode()) {
    // The circle IS the location here — whose it is, whether it pays, whether
    // anyone's been. So it takes everything the title doesn't need: one big
    // circle centred in the lot under the name.
    const bb = buildingBBox(b);
    const top = bb.minY + 11; // the title's line
    const band = Math.max(6, bb.maxY - top);
    const r = Math.max(7, Math.min(19, (bb.maxX - bb.minX - 5) / 2, (band - 4) / 2));
    const y = top + band / 2;
    return {
      r,
      sym: Math.max(9, Math.min(16, Math.round(r * 1.05))),
      underY: y + r + 4,
      centers: [[cx, y]]
    };
  }
...
function refreshLocations() {
  ...
  (mapState.blocks ?? []).forEach((bl) => (bl.buildings ?? []).forEach((b) => {
    ...
    if (isForcedMode()) {
      // The circle's meaning changes the moment somebody stands on it, so the
      // hover text is rewritten rather than baked in at draw time.
      const t = g.querySelector("title");
      if (t) t.textContent = forcedLocTitle(b);
      const taken = (b.slots ?? [])[0] != null;
      // My own outstanding jobs get a standing highlight — the six required
      // circles are the entire reason to drive anywhere, so they shouldn't
      // have to be hunted for.
      g.classList.toggle("um-forced-mine", !taken && b.owner === myIndex());
      g.classList.toggle("um-forced-shared-mine",
        !taken && Array.isArray(b.pair) && b.pair.includes(myIndex()));
      // A circle nobody at this table can ever use reads as scenery.
      g.classList.toggle("um-forced-dead", !taken && (
        (b.owner != null && !seatPlaying(b.owner)) ||
        (Array.isArray(b.pair) && !b.pair.some(seatPlaying))));
    }
...
function canUseLoc(b) {
  ...
  if (!Array.isArray(b.slots)) return false;
  // Forced mode: the circle names who may stand on it — one seat on a
  // required location, two on a shared one, and nobody else ever.
  if (b.owner != null && b.owner !== myIndex()) return false;
  if (Array.isArray(b.pair) && !b.pair.includes(myIndex())) return false;
...
function renderClock() {
  ...
  wrap.addEventListener("click", (event) => {
    ...
    socket.emit("landmark_mania_set_hour", { roomId: app.roomId, hour });
  });
...
// Star mode's second meter, sitting alongside the stress bar: every driver's
// rating from 0 to 10 stars, ticked at every half. A star location adds one;
// every landmark card still in hand at dawn takes half of one away. Your
// rating is what a delivery pays (rounded down), so it's public.
...
// The rating meter, alongside the stress bar: a stack of real stars, one per
// point of rating, filled from the bottom up to MY rating. A line runs through
// the waist of every star — land on it and you're half way into that star,
// land on the boundary above it and you've earned the whole thing. Star mode
// has seven and pays a star per star location; simple mode has five, opens on
// one, and earns them by the day (spread and a clean hand).
function renderRatingBar() {
  els.gameBoard.querySelector(".um-rating")?.remove();
  if (!(isStarMode() || isSimpleMode()) || !playersState.length) return;
  const max = Math.max(1, settingsState?.ratingMax ?? RATING_MAX);
  const wrap = document.createElement("div");
  wrap.className = "um-rating";
  const title = document.createElement("div");
  title.className = "um-stress-title";
  title.textContent = "RATING";
  wrap.appendChild(title);

  const cell = 21;            // height of one star's slice of the column
  const W = 30;               // width of the star column itself
  const H = cell * max;
  const svg = svgEl("svg", {
    class: "um-rating-stars",
    viewBox: `0 0 ${W + 26} ${H + 2}`,
    width: W + 26,
    height: H + 2
  });
  // One shared gradient does the half-fill: gold up to the waist, empty above.
  const defs = svgEl("defs", {}, svg);
  const grad = svgEl("linearGradient", {
    id: "um-star-half", x1: "0", y1: "1", x2: "0", y2: "0"
  }, defs);
  ...
  const cx = W / 2 + 1;
  // Star k counts from the bottom, so the column grows upward as it should.
  for (let k = 1; k <= max; k += 1) {
    const cy = 1 + H - (k - 0.5) * cell;
    const full = mine >= k;
    const half = !full && mine >= k - 0.5;
    const star = svgEl("polygon", {
      points: starPoints(cx, cy, cell * 0.46),
      class: "um-star"
    }, svg);
    if (full) star.style.fill = "#f0d68a";
    else if (half) star.style.fill = "url(#um-star-half)";
    // The waist line: the half-way mark of this star.
    svgEl("line", {
      x1: cx - cell * 0.5, y1: r1(cy), x2: cx + cell * 0.5, y2: r1(cy),
      class: "um-star-waist"
    }, svg);
  }

  // Everyone's marker, at their own rating. A whole rating parks on a star's
  // top boundary; a half parks on its waist line.
  playersState.forEach((p, i) => {
    const r = Math.max(0, Math.min(max, p.rating ?? 0));
    const y = 1 + H - r * cell;
    const g = svgEl("g", { class: "um-rating-pin" }, svg);
    svgEl("line", {
      x1: cx - cell * 0.55, y1: r1(y), x2: W + 4 + i * 5, y2: r1(y),
      class: "um-rating-pin-line", stroke: p.color
    }, g);
    const dot = svgEl("circle", {
      cx: W + 4 + i * 5, cy: r1(y), r: i === myIndex() ? 4 : 3,
      class: i === myIndex() ? "um-rating-dot um-rating-dot-mine" : "um-rating-dot"
    }, g);
    dot.style.fill = p.color;
    svgEl("title", {}, g).textContent =
      `${seatName(i)} — ${r} star${r === 1 ? "" : "s"}, so a delivery pays ${Math.floor(r)}`;
  });
...
function syncCars(cars) {
  ...
  carsState.forEach((t) => {
    if (t.spot == null) {
      // Undoing the drive ON to the board sends the car back to the garage —
      // stop any animation still running for it rather than leaving it
      // painting a hidden element (and holding `anyCarAnimating` true).
    ...
    // An undone move: the car was never there, so it doesn't drive back — it
    // is simply where it was, facing the way it was. Snapping also spares us
    // path-finding a reverse route whose end angle would point the wrong way.
    if (snapCarState?.truckId === t.id && snapCarState.spot === t.spot) {
      if (carAnim[t.id]) {
        cancelAnimationFrame(carAnim[t.id]);
        carAnim[t.id] = null;
      }
      ...
      carPos[t.id] = was?.spot === t.spot
        ? { x: was.x, y: was.y, angle: was.angle }
        : { x: spot.x, y: spot.y, angle: snapCarState.facing ?? spot.angle };
    ...
    if (prev == null) {
      ...
      if (entry?.path?.length >= 2) {
        // Driving on from the garage: undoing this puts the car back there,
        // which the off-board branch above handles on its own.
        carUndoPose[t.id] = { spot: null };
      ...
      // Remember exactly where and how the car is standing before it pulls
      // away — an undo puts it back here rather than driving the route in
      // reverse (which left it facing backwards).
      const here = carPos[t.id];
      carUndoPose[t.id] = here
        ? { spot: prev, x: here.x, y: here.y, angle: here.angle }
        : { spot: prev };
      carSpots[t.id] = t.spot;
      if (previewState?.truckId === t.id) clearPreview();
      const pending = pendingRoutes[t.id];
...
function commitRoute(routeIdx) {
  ...
  clearPreview();
  socket.emit("landmark_mania_move_truck", { roomId: app.roomId, truckId: car.id, spot, reds });
}
...
function builderGo() {
  ...
  builder = null;
  renderBuild();
  socket.emit("landmark_mania_move_truck", { roomId: app.roomId, truckId, spot: last.index, reds });
}
  ...
  if (locEl) {
    ...
    if (isDuplicateMode()) {
      ...
      if (onCircle && placeableBids().includes(bid)) {
        socket.emit("landmark_mania_place_token", { roomId: app.roomId, truckId: activeTruckId(), bid });
      } else if (!onCircle && completableBid() === bid) {
        socket.emit("landmark_mania_complete_ride", { roomId: app.roomId, truckId: activeTruckId() });
      }
      return;
    }
...
// Forced mode: how many required circles of this seat's color are still
// standing empty (null before the board has arrived).
function forcedLeft(seat) {
  if (!mapState) return null;
  let n = 0;
  let any = false;
  for (const bl of mapState.blocks ?? []) {
    for (const b of bl.buildings ?? []) {
      if (b.role !== "loc" || b.owner !== seat) continue;
      any = true;
      if (!(b.slots ?? []).includes(seat)) n += 1;
    }
  }
  return any ? n : null;
}
...
function renderScoreboard() {
  ...
  playersState.forEach((p, i) => {
    ...
    if (isNewMode()) {
      // The new modes keep the table quiet: the chip is a color and a name.
      // Tokens, time stones, whether they're sitting on a face-down stack and
      // the card they're showing all live in a panel you have to hover for,
      // and how many cards anyone has finished stays hidden either way.
      const nm = document.createElement("span");
      nm.className = "um-score-name";
      nm.textContent = p.name ?? seatName(i);
      chip.appendChild(nm);
      // Star mode scores in the open: a delivery pays the moment it lands.
      if (isStarMode()) {
        const pts = document.createElement("span");
        pts.className = "um-score-points";
        pts.textContent = String(p.points ?? 0);
        pts.title = "Points so far — deliveries pay on the spot, neighbourhoods pay at dawn";
        chip.appendChild(pts);
      }
      // Both star meters are public — but simple mode's is only half the
      // story, since the score is landmarks × stars and nobody sees anyone
      // else's card count until the end.
      // Forced mode: how many of that player's six required circles are still
      // empty. It's already public — the tokens are standing on the board —
      // and it's the only running score anyone can read, so it goes up top.
      if (isForcedMode()) {
        const left = forcedLeft(i);
        if (left != null) {
          const owed = document.createElement("span");
          owed.className = left ? "um-score-owed" : "um-score-owed um-score-owed-done";
          owed.textContent = left ? `⭕${left}` : "⭕✓";
          owed.title = left
            ? `${left} required circle${left === 1 ? "" : "s"} of their color still empty — ${left * (settingsState?.forcedMissPenalty ?? 1)} off their score if it ended now`
            : "Every required circle of their color is covered";
          chip.appendChild(owed);
        }
      }
      if (isStarMode() || isSimpleMode()) {
        const rate = document.createElement("span");
        rate.className = "um-score-rating";
        rate.textContent = `${num(p.rating ?? 0)}★`;
        rate.title = isSimpleMode()
          ? `${num(p.rating ?? 0)} stars — the multiplier on however many landmarks they finish`
          : `Rating ${p.rating ?? 0} — a delivery pays ${Math.floor(p.rating ?? 0)}`;
...
function playerHoverCard(p, seat) {
  ...
  // Landmark mode: the colors this player has claimed, as swatches carrying
  // the place they took. (Star and simple modes show nothing here — which
  // neighbourhoods anyone has reached today is written on the board itself.)
  if (isLandmarkMode()) {
    const swatchRow = document.createElement("div");
    swatchRow.className = "um-peek-row";
...
// Simple mode's day tracker: one dot per neighbourhood, lit where I have a
// token standing today, and a readout of what this dawn is about to pay. The
// day is worth up to two stars — one for the spread (all five, or a half for
// four), one for a clean hand (empty, or a half for nothing face down) — and
// the score at the end is landmarks × stars, so the two halves only matter
// together.
function renderSimpleTracker(wrap) {
  const seat = myIndex();
  const held = new Set();
  for (const bl of mapState?.blocks ?? []) {
    for (const b of bl.buildings ?? []) {
      if (b.role === "loc" && b.hood != null && (b.slots ?? []).includes(seat)) held.add(b.hood);
    }
  }
  const total = hoodsState.length;
  const row = document.createElement("div");
  row.className = "um-prizes um-simple-track";
  hoodsState.forEach((h) => {
    const dot = document.createElement("span");
    dot.className = held.has(h.id) ? "um-prize" : "um-prize um-prize-gone";
    dot.style.background = h.color;
    dot.title = held.has(h.id)
      ? `${h.name} — a token is standing here`
      : `${h.name} — no token here yet today`;
    const lit = (on) => {
      for (const bl of mapState?.blocks ?? []) {
        for (const b of bl.buildings ?? []) {
          if (b.hood !== h.id) continue;
  ...
  // What the day would pay if it ended right now: the spread star and the
  // clean-hand star, each whole, half or nothing.
  const rides = myPlayer()?.rides ?? [];
  const down = rides.filter((r) => r.faceDown).length;
  const spread = total > 0 && held.size >= total ? 1 : total > 1 && held.size >= total - 1 ? 0.5 : 0;
  const hand = rides.length === 0 ? 1 : down === 0 ? 0.5 : 0;

  const pill = (value, label, why) => {
    const el = document.createElement("span");
    el.className = value > 0 ? "um-day-star" : "um-day-star um-day-star-none";
    el.textContent = value ? `${label} +${num(value)}★` : `${label} —`;
    el.title = why;
    row.appendChild(el);
  };
  pill(spread, "🏘️",
    `Neighbourhoods: ${held.size} of ${total} today. All ${total} pays a whole star at dawn, ${total - 1} pays a half.`);
  pill(hand, "🎴",
    down > 0
      ? `${down} card${down === 1 ? "" : "s"} still face down — they expire at dawn and pay no star. Nothing face down is a half star; an empty hand is a whole one.`
      : rides.length
      ? `${rides.length} card${rides.length === 1 ? "" : "s"} face up — half a star at dawn. Deliver them all for the whole one.`
      : "An empty hand at dawn is a whole star.");
  wrap.appendChild(row);
}

// Forced mode's job list. Two rows: my six REQUIRED circles — the entire
// reason to leave the house, since each one left empty costs a point — and
// the eight SPLIT circles with my name on them, which are where all the
// tokens and time stones in the game come from. Each pip lights up while the
// circle is still standing there and greys out once it's gone, and hovering
// one picks the building out on the board.
function renderForcedTracker(wrap) {
  const seat = myIndex();
  const mine = [];
  const shared = [];
  for (const bl of mapState?.blocks ?? []) {
    for (const b of bl.buildings ?? []) {
      if (b.role !== "loc") continue;
      if (b.owner === seat) mine.push(b);
      else if (Array.isArray(b.pair) && b.pair.includes(seat)) shared.push(b);
    }
  }
  if (!mine.length && !shared.length) return;
  // Timed ones first, so the two that are shut right now sit together.
  const rank = (b) => (b.period === "day" ? 0 : b.period === "night" ? 1 : 2);
  mine.sort((a, b) => rank(a) - rank(b) || (a.name ?? "").localeCompare(b.name ?? ""));
  shared.sort((a, b) => (a.locType ?? "").localeCompare(b.locType ?? ""));

  const lit = (b, on) => els.gameBoard
    .querySelector(`.tm-building[data-bldg="${b.bid}"]`)?.classList.toggle("um-ride-hover", on);

  const makeRow = (list, cls, pipFor) => {
    const row = document.createElement("div");
    row.className = `um-prizes um-forced-track ${cls}`;
    list.forEach((b) => {
      const pip = document.createElement("span");
      const { done, title, text, color } = pipFor(b);
      pip.className = done ? "um-forced-pip um-forced-pip-done" : "um-forced-pip";
      if (!done && !locOpen(b, timeState)) pip.classList.add("um-forced-pip-shut");
      pip.style.setProperty("--pip", color);
      pip.textContent = text;
      pip.title = title;
      pip.addEventListener("mouseenter", () => lit(b, true));
      pip.addEventListener("mouseleave", () => lit(b, false));
      row.appendChild(pip);
    });
    wrap.appendChild(row);
  };
  ...
  if (mine.length) {
    const left = mine.filter((b) => !(b.slots ?? []).includes(seat)).length;
    const miss = settingsState?.forcedMissPenalty ?? 1;
    const head = document.createElement("div");
    head.className = "um-forced-head";
    head.textContent = left
      ? `⭕ ${left} to go · −${left * miss}`
      : "⭕ all covered";
    head.title = left
      ? `${left} required circle${left === 1 ? "" : "s"} of your color still empty — that's ${left * miss} point${left * miss === 1 ? "" : "s"} off if the game ended now.`
      : "Every required circle of your color is covered — nothing left to lose here.";
    if (!left) head.classList.add("um-forced-head-done");
    wrap.appendChild(head);
    makeRow(mine, "um-forced-track-req", (b) => ({
      done: (b.slots ?? []).includes(seat),
      color: seatColor(seat),
      text: b.emoji ?? "•",
      title: forcedLocTitle(b)
    }));
  }
  if (shared.length) {
    makeRow(shared, "um-forced-track-share", (b) => {
      const taken = (b.slots ?? [])[0];
      const other = b.pair.find((s) => s !== seat);
      return {
        done: taken != null,
        color: seatColor(other),
        text: b.locType === "timestone" ? "⬟" : "💰",
        title: forcedLocTitle(b)
      };
    });
  }
...
function renderPlayerPanel() {
  ...
  // Star mode keeps the panel down to what you actually hold in hand. The
  // stress marker and the star rating are already on their bars, points sit
  // next to every name up top, and fines are nobody's business until scoring
  // — so none of them are repeated here.
  if (!isStarMode() && !isSimpleMode()) {
    ...
    redLost.title = isLandmarkMode()
      ...
      : isForcedMode()
      ? `Fines paid — tokens lost to failed dice, and a fine you have no tokens for counts double instead of costing anything. At game end the fewest gains ${settingsState?.forcedFineSwing ?? 1} and the most loses it, and fines break ties.`
  ...
  // Simple mode: the stress marker and the star rating are both on their own
  // bars, but fines are worth watching — the swing at the end is worth real
  // points, and one you can't cover costs half a star on the spot.
  if (isSimpleMode()) {
    const redLost = document.createElement("div");
    redLost.className = "um-stat um-stat-red";
    redLost.textContent = `🚦 ${me.redTokensLost ?? 0} fines`;
    redLost.title = `Fines paid — tokens lost to failed dice. At game end the fewest gains ${settingsState?.simpleFineSwing ?? 3} and the most loses it. A fine you have no tokens for costs half a star instead.`;
    stats.appendChild(redLost);
  }
  ...
  // (Star mode shows no spread tracker at all — which neighbourhoods you've
  // reached today, and what they'll pay, is yours to read off the board.)
  if (isLandmarkMode() && hoodsState.length) renderColorPrizes(wrap);
  // Simple mode's day tracker: the five neighbourhoods, lit where I already
  // hold a token, plus the two card decks and who took today's traffic bonus.
  if (isSimpleMode() && hoodsState.length) renderSimpleTracker(wrap);
  // Forced mode's job list: my six required circles and the eight split ones
  // I'm named on, each lit where it's still there for the taking.
  if (isForcedMode()) renderForcedTracker(wrap);
  ...
  // (7 for the first player, then 5, 3, 1). Star, simple and forced modes have
  // no upgrades at all, so the board doesn't show.
  if (!isStarMode() && !isSimpleMode() && !isForcedMode()) {
    const row = document.createElement("div");
  ...
  if (me.rides?.length) {
    ...
    if (isNewMode() && down) {
      ...
      const waiting = `${down} landmark card${down > 1 ? "s" : ""} waiting. You hold ${visibleCardCap()} face up — the pile flips one up whenever a turn ends with room for it.`;
      card.title = isStarMode()
        ? `${waiting} Every card still in hand at dawn is late and costs you half a star.`
        : isSimpleMode()
        ? `${waiting} A card still face down when the day ends (6am) expires — and costs you the clean-hand star as well.`
        : `${waiting} Every card still down when a night ends (6am) costs ${per} point.`;
      card.classList.add("um-ride-urgent");
      row.appendChild(card);
    }
    me.rides.forEach((r) => {
      if (r.faceDown) {
        if (isNewMode()) return; // already counted in the pile above
      ...
      if (isForcedMode() && !isCornerPaint()) {
        // This forced-mode board carries names and nothing else, so its cards
        // do too — a picture here would point at something not drawn out
        // there. (The corner layout puts the pictures back, and so does this.)
        card.classList.add("um-ride-card-text");
      ...
      card.title = isForcedMode()
        ? `Drive to ${b?.name ?? "this location"} to complete the card — ${settingsState?.forcedCardPoints ?? 1} point. You can complete it OR use the circle there, never both on one visit.`
        : isStarMode()
        ? `Drive to ${b?.name ?? "this landmark"} to deliver — pays ${Math.floor(myPlayer()?.rating ?? 0)} at your current rating`
        : isSimpleMode()
        ? `Drive to ${b?.name ?? "this landmark"} to complete the card — every completed landmark multiplies your stars at the end (${myPlayer()?.rating ?? 0}★ right now), and clearing your hand by dawn is worth a star of its own`
        : isLandmarkMode()
...
const handoutKind = () => (isNewMode() && isNightNow() ? "beg" : "welfare");
...
function sleepButton() {
  ...
  btn.title = "Sit the turn out to sleep (night only): stress drops all the way down. It costs you the whole turn and does not move the clock.";
  btn.addEventListener("click", () => {
    if (btn.disabled || !app.roomId) return;
    withHoodChoices((hoodChoices) =>
      socket.emit("landmark_mania_sleep", { roomId: app.roomId, hoodChoices }));
  });
  wrap.append(btn);
...
function canNap() {
  return isNewMode() && isMyTurn() && winnerState == null && turnTruck == null &&
...
function napButton() {
  ...
  btn.title = "Sit the turn out for a daytime nap: the stress marker moves two steps down the bar. It costs you the whole turn and does not move the clock.";
  btn.addEventListener("click", () => {
    if (btn.disabled || !app.roomId) return;
    withHoodChoices((hoodChoices) =>
      socket.emit("landmark_mania_nap", { roomId: app.roomId, hoodChoices }));
  });
  wrap.append(btn);
...
function keepGoingButton() {
  ...
  btn.disabled = diceAnimating || anyCarAnimating() || flipping;
  btn.addEventListener("click", () => {
    if (!btn.disabled && app.roomId) socket.emit("landmark_mania_keep_going", { roomId: app.roomId });
  });
  return btn;
}
...
function ensureActionBar() {
  ...
  if (!bar) {
    ...
    undoBtn.addEventListener("click", () => {
      if (!undoBtn.disabled && app.roomId) socket.emit("landmark_mania_undo", { roomId: app.roomId });
    });
    ...
    endBtn.addEventListener("click", () => {
      ...
      withHoodChoices((hoodChoices) =>
        socket.emit("landmark_mania_end_turn", { roomId: app.roomId, hoodChoices }));
    });
...
function renderControls() {
  ...
  // The alternative modes, each a single switch that re-tunes the whole board.
  // Flipping one re-deals, so they live right here next to New map rather than
  // only inside the tuning panel. They're exclusive — turning one on turns the
  // others off, and turning them all off restores the classic board.
  const modeToggle = (key, on, label, title) => {
    const wrap = document.createElement("label");
    wrap.className = "um-mode-toggle";
    wrap.title = title;
    ...
    box.addEventListener("change", () => {
      ...
      MODE_KEYS.forEach((k) => { next[k] = false; });
      next[key] = box.checked;
      socket.emit("landmark_mania_tune", { roomId: app.roomId, settings: next });
    });
    const text = document.createElement("span");
    text.textContent = label;
    wrap.append(box, text);
    if (on) wrap.classList.add("tm-active");
    els.hand.appendChild(wrap);
  };
  modeToggle("landmarkMode", isLandmarkMode(), "🗽 Landmark",
    "Landmark mode: 50 locations, 7 scoring colors, landmark cards discovered one at a time, 7 day / 7 night locations, 4 upgrade stacks behind named windows. Everything else tunes itself — switching back restores your classic board.");
  modeToggle("starMode", isStarMode(), "⭐ Star",
    "Star mode: 52 locations with single circles that wipe every dawn, 8 day / 8 night locations, 5 neighbourhoods scored fresh each day, and a 7-star rating that decides what every delivery pays. Everything else tunes itself — switching back restores your classic board.");
  modeToggle("simpleMode", isSimpleMode(), "🚦 Simple",
    "Simple mode: 50 locations in 5 even neighbourhoods (1 time stone, 1 token, 2 discovery, 6 landmarks each), single circles that wipe every dawn, 6 day / 6 night locations, no upgrades. Every day's end pays up to two of your five stars — one for a token in all five neighbourhoods, one for an empty hand — and the score is landmarks × stars. Everything else tunes itself — switching back restores your classic board.");
  modeToggle("forcedMode", isForcedMode(), "⭕ Forced",
    "Forced mode: 50 locations in 7 neighbourhoods that carry no rules at all, each with a name, an emoji and ONE circle. 30 are required — the circle is one player's color, only they may use it, it pays nothing, and leaving it empty costs them a point (2 of each player's 6 open only by day, 2 only at night). The other 20 are split between two players: whoever gets there takes 4 tokens / 6 stones and the other is handed half on the spot. Hand of 2 cards, refilled each turn, a point each; tokens never leave the board; ±1 on fines. Everything else tunes itself — switching back restores your classic board.");

  // Forced mode's view sub-menu: which of the three channels are carrying
  // player color right now. Purely a way of LOOKING at the board — it redraws
  // on the spot, never touches the room's settings and never re-deals, so it's
  // safe mid-game and personal to this browser.
  if (isForcedMode()) {
    const paintWrap = document.createElement("span");
    paintWrap.className = "um-paint-wrap";
    paintWrap.title = "Where a location shows whose it is. Any combination — with outline and fill both on the neighbourhood colors have nowhere left to show, and with all three off nothing does.";
    const paintLab = document.createElement("span");
    paintLab.className = "um-paint-label";
    paintLab.textContent = "View";
    paintWrap.appendChild(paintLab);
    FORCED_PAINT.forEach(([key, label, blurb]) => {
      // Corner is a layout, not a channel: while it's on the four channels
      // above it do nothing, and the menu says so by greying them out.
      const overridden = key !== "corner" && forcedPaint.corner === true;
      const lab = document.createElement("label");
      lab.className = `um-paint${forcedPaint[key] ? " tm-active" : ""}${overridden ? " um-paint-off" : ""}`;
      lab.title = overridden ? `${blurb}  (Corner is on, so this is ignored.)` : blurb;
      const box = document.createElement("input");
      box.type = "checkbox";
      box.checked = forcedPaint[key] === true;
      box.disabled = overridden;
      box.addEventListener("change", () => {
        forcedPaint = { ...forcedPaint, [key]: box.checked };
        try {
          localStorage.setItem("umForcedPaint", JSON.stringify(forcedPaint));
        } catch {
          /* a browser refusing storage just forgets the choice next time */
        }
        // Redraws the buildings layer from the current map; every bit of game
        // state re-applies itself through refreshLocations. Corner also
        // changes what the cards show and what the rest of the menu is
        // allowed to do, so it redraws the controls as well.
        applyBuildingScale();
        if (key === "corner") {
          renderControls();
          renderPlayerPanel();
        } else {
          lab.classList.toggle("tm-active", box.checked);
        }
      });
      const text = document.createElement("span");
      text.textContent = label;
      lab.append(box, text);
      paintWrap.appendChild(lab);
    });
    els.hand.appendChild(paintWrap);
  }
  ...
  dial.addEventListener("change", () => {
    if (app.roomId) socket.emit("landmark_mania_set_speed", { roomId: app.roomId, speed: Number(dial.value) });
  });
  ...
  aiSelect.addEventListener("change", () => {
    ...
    socket.emit("landmark_mania_set_opponents", { roomId: app.roomId, count: Number(aiSelect.value) });
  });
...
function renderResults() {
  ...
  if (resultsState.mode === "star") {
    renderStarResults(table);
    panel.appendChild(table);
    finishResults(panel, overlay);
    return;
  }
  if (resultsState.mode === "simple") {
    renderSimpleResults(table);
    panel.appendChild(table);
    finishResults(panel, overlay);
    return;
  }
  if (resultsState.mode === "forced") {
    renderForcedResults(table);
    panel.appendChild(table);
    finishResults(panel, overlay);
    return;
  }
...
// Star mode's chart: almost everything already scored live, plus the two
// split bonuses that land at the end.
function renderStarResults(table) {
  const winners = resultsState.winners ?? [];
  const head = document.createElement("tr");
  ["", "⭐ Rating", "🏁 Scored", "🗽 Delivered", "🚦 Fines", "Total"].forEach((h) => {
    const th = document.createElement("th");
    th.textContent = h;
    head.appendChild(th);
  });
  ...
  (resultsState.perPlayer ?? []).forEach((r, i) => {
    ...
    rating.textContent = `${num(r.rating)}★`;
    rating.title = "Where the rating finished — what a last-minute delivery would have paid";
    tr.appendChild(rating);

    const live = document.createElement("td");
    live.textContent = num(r.live);
    live.title = "Points banked during play: deliveries as they landed, plus each day's neighbourhood spread";
    tr.appendChild(live);

    const done = document.createElement("td");
    done.innerHTML = `${r.landmarks ?? 0}${r.mostBonus ? " ⭐" : ""} <span class="um-results-pts ${r.mostBonus ? "um-results-pos" : ""}">${r.mostBonus ? `+${num(r.mostBonus)}` : "±0"}</span>`;
    if (r.mostBonus) done.title = `Tied for most cards delivered — the bonus was split`;
    tr.appendChild(done);

    const fines = document.createElement("td");
    fines.innerHTML = `${r.fines ?? 0} <span class="um-results-pts ${r.fineBonus ? "um-results-pos" : ""}">${r.fineBonus ? `+${num(r.fineBonus)}` : "±0"}</span>`;
    if (r.fineBonus) fines.title = "Fewest fines paid — the bonus was split between everyone tied";
    tr.appendChild(fines);

    const total = document.createElement("td");
    total.className = "um-results-total";
    total.textContent = num(r.total);
    tr.appendChild(total);
...
// Simple mode's chart: nothing was banked along the way. The whole game is
// landmarks × stars, written out as the multiplication it is, plus the fine
// swing.
function renderSimpleResults(table) {
  const winners = resultsState.winners ?? [];
  const head = document.createElement("tr");
  ["", "🗽 Landmarks", "⭐ Stars", "🂠 Expired", "🚦 Fines", "Total"].forEach((h) => {
    const th = document.createElement("th");
    th.textContent = h;
    head.appendChild(th);
  });
  ...
  (resultsState.perPlayer ?? []).forEach((r, i) => {
    ...
    const lm = document.createElement("td");
    lm.textContent = String(r.landmarks ?? 0);
    lm.title = "Landmark cards completed — one half of the score";
    tr.appendChild(lm);

    // Stars, with the multiplication spelled out beside them.
    const stars = document.createElement("td");
    stars.innerHTML = `${num(r.stars ?? 0)}★ <span class="um-results-pts">=${num(r.base ?? 0)}</span>`;
    stars.title = r.starsLost
      ? `${num(r.stars ?? 0)} stars × ${r.landmarks ?? 0} landmarks. ${num(r.starsLost)} lost along the way to fines there were no tokens for.`
      : `${num(r.stars ?? 0)} stars × ${r.landmarks ?? 0} landmarks`;
    tr.appendChild(stars);

    const exp = document.createElement("td");
    exp.textContent = r.expired ? `🂠${r.expired}` : "—";
    exp.title = `${r.expired ?? 0} card${r.expired === 1 ? "" : "s"} expired face down at the end of a day — each one also cost that day's clean-hand star`;
    tr.appendChild(exp);

    // Fines: the end-game swing, gained by the fewest and lost by the most.
    const fines = document.createElement("td");
    ...
    fines.title = "Tokens lost to failed dice — the fewest gains the swing, the most loses it";
    tr.appendChild(fines);
...
// Forced mode's chart, and there is almost nothing in it: cards completed,
// required circles left empty, and the fine swing.
function renderForcedResults(table) {
  const winners = resultsState.winners ?? [];
  const head = document.createElement("tr");
  ["", "🎴 Cards", "⭕ Required", "🚦 Fines", "Total"].forEach((h) => {
    const th = document.createElement("th");
    th.textContent = h;
    head.appendChild(th);
  });
  ...
  (resultsState.perPlayer ?? []).forEach((r, i) => {
    ...
    cards.innerHTML = `${r.cards ?? 0} <span class="um-results-pts um-results-pos">+${r.cardPts ?? 0}</span>`;
    cards.title = `${r.cards ?? 0} landmark card${r.cards === 1 ? "" : "s"} completed`;
    tr.appendChild(cards);

    // Covered / total, with what the empties cost.
    const req = document.createElement("td");
    const missPts = r.missPts ?? 0;
    req.innerHTML = `${r.covered ?? 0}/${r.required ?? 0} <span class="um-results-pts ${missPts ? "um-results-neg" : ""}">${missPts ? `−${missPts}` : "±0"}</span>`;
    req.title = r.missed
      ? `${r.missed} of their ${r.required} required circles never got covered`
      : "Every required circle covered";
    tr.appendChild(req);

    const fines = document.createElement("td");
    const adj = r.fineAdj > 0 ? `+${r.fineAdj}` : String(r.fineAdj);
    fines.innerHTML = `${r.fines ?? 0} <span class="um-results-pts ${r.fineAdj < 0 ? "um-results-neg" : r.fineAdj > 0 ? "um-results-pos" : ""}">${r.fineAdj === 0 ? "±0" : adj}</span>`;
    fines.title = "Tokens lost to failed dice, counting double for any the player had no tokens to pay — the fewest gains the swing, the most loses it, and it breaks ties";
    tr.appendChild(fines);

    const total = document.createElement("td");
    total.className = "um-results-total";
    total.textContent = num(r.total);
    if (r.tiebroken) total.title = "Level on points — beaten on fines paid";
    tr.appendChild(total);
...
// Both star meters deal in halves (ratings, day-end gains, split bonuses) —
// print them without a trailing ".0" on the whole numbers.
...
function finishResults(panel, overlay) {
  ...
  if (resultsState.mode === "star") {
    const hint = document.createElement("div");
    hint.className = "um-results-hint";
    const table = s?.hoodSpread ?? HOOD_SPREAD_SCORE;
    hint.textContent = `Deliveries paid your rating rounded down, the moment they landed · Each dawn paid for the neighbourhoods you had a token in (${table.map((v, i) => `${i}→${v}`).join(", ")}) and then swept the board · A star location was +1 star, every card still in hand at dawn −0.5 · At the end: ${s?.starMostBonus ?? 6} split between everyone tied for most delivered, and ${s?.starFineBonus ?? 6} split between everyone tied for fewest fines`;
    panel.appendChild(hint);
  }
  if (resultsState.mode === "simple") {
    const hint = document.createElement("div");
    hint.className = "um-results-hint";
    hint.textContent = `Score = landmarks completed × stars · Each day's end paid up to two stars: a whole one for a token in all ${hoodsState.length} neighbourhoods (a half for ${hoodsState.length - 1}), and a whole one for an empty hand (a half for nothing face down) — then cards still face down expired and the board swept · A fine you had no tokens for cost half a star · Fines at the end: ±${s?.simpleFineSwing ?? 3} to the fewest and the most paid`;
    panel.appendChild(hint);
  }
  if (resultsState.mode === "forced") {
    const hint = document.createElement("div");
    hint.className = "um-results-hint";
    hint.textContent = `+${s?.forcedCardPoints ?? 1} per landmark card completed · −${s?.forcedMissPenalty ?? 1} for every required circle of your color still empty (six each, and they never came off the board) · The twenty split circles paid nothing at the end — they were the tokens and time stones that got you around, and half of each went to the other name on the circle · Fines: ±${s?.forcedFineSwing ?? 1} to the fewest and the most paid, with a fine you had no tokens for counting double, and fines breaking ties`;
    panel.appendChild(hint);
  }
  if (resultsState.mode === "landmark") {
    const hint = document.createElement("div");
  ...
  againBtn.addEventListener("click", () => {
    ...
    socket.emit("landmark_mania_regenerate", { roomId: app.roomId });
  });
...
// Forced mode: somebody used a split circle, so the other name on it just got
// handed half. That's the one thing in the mode that happens to you while
// you're doing something else, so it says so out loud.
function showShareToast(share) {
  document.querySelector(".um-share-toast")?.remove();
  const div = document.createElement("div");
  div.className = "um-share-toast";
  const mine = share.to === myIndex();
  div.style.borderColor = seatColor(share.to);
  const what = share.kind === "stones"
    ? `${share.amount} time stone${share.amount === 1 ? "" : "s"} ⬟`
    : `${share.amount} token${share.amount === 1 ? "" : "s"} 💰`;
  div.textContent = mine
    ? `${seatName(share.from)} went shopping — you get ${what}`
    : `${seatName(share.from)} → ${seatName(share.to)}: ${what}`;
  if (mine) div.classList.add("um-share-toast-mine");
  document.body.appendChild(div);
  setTimeout(() => div.remove(), 2600 / speedMult);
}
...
// Star mode's own numbers.
// (The rating cap isn't here — star mode fixes the meter at seven stars.)
const TUNE_STAR_FIELDS = [
  ["startingRating", "⭐ Starting rating"],
  ["starMostBonus", "🗽 Most delivered (split)"],
  ["starFineBonus", "🚦 Fewest fines (split)"]
];
// Simple mode's own numbers.
const TUNE_SIMPLE_FIELDS = [
  ["simpleStartStars", "⭐ Starting stars"],
  ["simpleFineSwing", "🚦 Fine swing (±)"],
  ["begTokens", "🥺 Beg tokens (night)"]
];
// Forced mode's own numbers.
const TUNE_FORCED_FIELDS = [
  ["forcedCardPoints", "🎴 Points per landmark card"],
  ["forcedMissPenalty", "⭕ Per required location missed"],
  ["forcedFineSwing", "🚦 Fine swing (±)"],
  ["forcedCards", "🖐️ Cards in hand"],
  ["begTokens", "🥺 Beg tokens (night)"]
];
...
// The classic scoring numbers the alternative modes replace.
const TUNE_CLASSIC_SCORING = ["ridePoints", "redPenalty"];
// The mode switches, in the order they arrived. Flipping any one on clears the
// rest — they're exclusive. (Server: sanitizeSettings.)
const MODE_KEYS = ["landmarkMode", "starMode", "simpleMode", "forcedMode"];
...
function openTuning() {
  ...
  if (!tuningsRequested) {
    ...
    socket.emit("landmark_mania_list_settings");
  }
  renderControls();
}
...
function buildTuningList() {
  ...
  savedTunings.forEach((item) => {
    ...
    ok.addEventListener("click", () => {
      socket.emit("landmark_mania_rename_settings", { roomId: app.roomId, settingsId: item.id, name: name.value.trim() });
    });
    ...
    apply.addEventListener("click", () => {
      socket.emit("landmark_mania_load_settings", { roomId: app.roomId, settingsId: item.id });
      closeTuning();
      renderControls();
    });
    ...
    if (canSaveTunings) {
      ...
      del.addEventListener("click", () => {
        if (del.dataset.armed) {
          socket.emit("landmark_mania_delete_settings", { roomId: app.roomId, settingsId: item.id });
          return;
        }
        del.dataset.armed = "1";
...
function renderTuning() {
  ...
  title.textContent = "Landmark Mania tuning — applying re-deals the board";
  panel.appendChild(title);

  if (savedTunings.length) panel.appendChild(buildTuningList());

  // The alternative modes, each a single switch that tunes the rest of the
  // board for you. They're exclusive: turning one on turns the others off, and
  // everything any of them owns goes grey below.
  const lm = !!tuneDraft.landmarkMode;
  const st = !!tuneDraft.starMode;
  const sp = !!tuneDraft.simpleMode;
  const fo = !!tuneDraft.forcedMode;
  const anyMode = lm || st || sp || fo;
  const modeSwitch = (key, on, label, blurb) => {
    const row = document.createElement("label");
    row.className = `um-tune-mode${on ? " um-tune-mode-on" : ""}`;
    const box = document.createElement("input");
    box.type = "checkbox";
    box.checked = on;
    box.addEventListener("change", () => {
      // Only one mode at a time.
      MODE_KEYS.forEach((k) => { tuneDraft[k] = false; });
      tuneDraft[key] = box.checked;
      renderTuning();
    });
    const text = document.createElement("span");
    text.className = "um-tune-mode-text";
    const b = document.createElement("b");
    b.textContent = label;
    const small = document.createElement("small");
    small.textContent = blurb;
    text.append(b, document.createElement("br"), small);
    row.append(box, text);
    panel.appendChild(row);
  };
  modeSwitch("landmarkMode", lm, "🗽 Landmark mode",
    "50 locations · 7 scoring colors, 4 locations each · landmark cards discovered one at a time, the rest face down and bleeding a point every night · 7 day / 7 night locations · 4 upgrade stacks behind named windows · no neighbourhoods, no destress locations.");
  modeSwitch("starMode", st, "⭐ Star mode",
    "52 locations · single circles that wipe every dawn · 8 day / 8 night locations · 5 neighbourhoods scored fresh each day (3 of them pays 1, 4 pays 3, all 5 pays 6) · a 7-star rating that sets what each delivery pays, docked half a star per card left late · no upgrades, no destress locations.");
  modeSwitch("simpleMode", sp, "🚦 Simple mode",
    "50 locations · 5 even neighbourhoods of 1 time stone, 1 token, 2 discovery and 6 landmarks · SINGLE circles, wiped every dawn · 6 day / 6 night of the 20 token locations, spread a hood at a time so none is ever shut, the other 8 open whenever · a five-star meter opening on one: each day's end pays a whole star for a token in all five neighbourhoods (half for four) and a whole star for an empty hand (half for nothing face down), and an unpayable fine costs half · score = landmarks completed × stars, ±3 on fines · no upgrades, no destress locations.");
  modeSwitch("forcedMode", fo, "⭕ Forced mode",
    "50 locations in 7 neighbourhoods that mean nothing at all — they're there so the board has shapes to steer by · every location a name, an emoji and ONE circle · 30 REQUIRED: the circle is one player's color, only they may ever use it, it pays nothing, and every one left empty at the end costs them a point (six each — two open only by day, two only at night, two whenever) · 20 SHARED: the circle is split between two players' colors, two locations for each of the ten pairs, one paying 4 tokens and one paying 6 time stones — whoever gets there takes the lot and the other name on the circle is handed half on the spot · a hand of two landmark cards, refilled at the end of every turn, a point each, and a card can point at a circle you could also use — you have to pick one and drive back for the other · tokens NEVER come off the board and nothing happens at the end of a day · a fine you can't pay counts double on your fines · ±1 to the fewest and most fines paid, and fines break ties.");
  ...
  modeLab.className = anyMode ? "um-tune-row um-tune-off" : "um-tune-row";
  ...
  upLab.className = anyMode ? "um-tune-row um-tune-off" : "um-tune-row";
  ...
  timedLab.className = anyMode ? "um-tune-row um-tune-off" : "um-tune-row";
  ...
  TUNE_FIELDS.forEach(([key, label]) => {
    ...
    if (anyMode && TUNE_CLASSIC_SCORING.includes(key)) return;
    addField(label, tuneDraft[key] ?? 0, (v) => {
      tuneDraft[key] = Number(v);
    }, anyMode && LANDMARK_OWNED.has(key));
  });
  const subhead = (text) => {
    const head = document.createElement("div");
  ...
  if (lm) {
    subhead("🗽 Landmark scoring");
    TUNE_LANDMARK_FIELDS.forEach(([key, label]) => {
      addField(label, tuneDraft[key] ?? 0, (v) => { tuneDraft[key] = Number(v); });
    });
  }
  if (st) {
    subhead("⭐ Star scoring");
    TUNE_STAR_FIELDS.forEach(([key, label]) => {
      addField(label, tuneDraft[key] ?? 0, (v) => { tuneDraft[key] = Number(v); });
    });
    // The neighbourhood-spread payout table, one field per possible count.
    subhead("🏘️ Points per neighbourhood spread");
    (tuneDraft.hoodSpread ?? HOOD_SPREAD_SCORE).forEach((v, i) => {
      addField(`${i} neighbourhood${i === 1 ? "" : "s"}`, v, (nv) => {
        (tuneDraft.hoodSpread ??= [...HOOD_SPREAD_SCORE])[i] = Number(nv);
      });
    });
  }
  if (sp) {
    subhead("🚦 Simple scoring");
    TUNE_SIMPLE_FIELDS.forEach(([key, label]) => {
      addField(label, tuneDraft[key] ?? 0, (v) => { tuneDraft[key] = Number(v); });
    });
  }
  if (fo) {
    subhead("⭕ Forced scoring");
    TUNE_FORCED_FIELDS.forEach(([key, label]) => {
      addField(label, tuneDraft[key] ?? 0, (v) => { tuneDraft[key] = Number(v); });
    });
  }
  ...
  if (canSaveTunings) {
    ...
    save.addEventListener("click", () => {
      socket.emit("landmark_mania_save_settings", {
        roomId: app.roomId,
  ...
  apply.addEventListener("click", () => {
    socket.emit("landmark_mania_tune", { roomId: app.roomId, settings: tuneDraft });
    closeTuning();
    renderControls();
  });
...
socket.on("landmark_mania_settings", ({ settings, canSave } = {}) => {
...
socket.on("landmark_mania_settings_error", ({ message } = {}) => {
...
els.hand.addEventListener("click", (event) => {
  ...
  switch (btn.dataset.action) {
    case "regen":
      ...
      break;
    case "mixup":
      socket.emit("landmark_mania_mix_up", { roomId: app.roomId });
      break;
    case "togglebar":
    ...
    case "skipturn":
      withHoodChoices((hoodChoices) =>
        socket.emit("landmark_mania_skip_turn", {
...
function boardSvg() {
  const svg = svgEl("svg", {
    ...
    role: "img",
    "aria-label": "Landmark Mania city map"
  });
  svgEl("path", {
    class: "tm-ground",
...
function setTurnStatus() {
  ...
  if (turnActed) {
    ...
    els.turnStatus.textContent = canVisit && canComplete
      ...
      : b?.locType === "star"
      ? `Your turn — click ${b.name ?? "this location"} for a star (rating ${myPlayer()?.rating ?? 0} → ${(myPlayer()?.rating ?? 0) + 1})`
      : b?.locType === "upgrade"
...
export const landmarkMania = {
  ...
  handleState(payload, resetGameUi) {
    ...
    if (sameMap) {
      ...
      // Forced mode: a split circle just paid its other half out.
      if (shareState && shareState.seq !== lastShareSeq) {
        lastShareSeq = shareState.seq;
        showShareToast(shareState);
      }
      syncCars(um.trucks);
      renderScoreboard();
      renderPlayerPanel();
      ...
      lastShareSeq = um.share?.seq ?? lastShareSeq;
